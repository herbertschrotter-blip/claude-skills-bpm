...
| Situation | Verhalten |
|-----------|-----------|
| "tracker neu: ..." | Schnellmodus oder Fragemodus (Auswahlfrage!) + Chat erstellt + Custom Fields |
| "tracker start: ..." | Status auf `in progress` + Parent-Check + Pro-Task-Quittung |
| "starte BPM-NNN" / "fangen wir mit BPM-NNN an" | Auswahlfrage: Beide / Nur Subtask / Nein |
...
| "tracker split: ..." | Vorschlag + Auswahlfrage zur Bestätigung + NN.NN-Nummerierung |
...
| code-erstellen nach Commit | NUR vorschlagen (per Auswahlfrage) |
| chat-wechsel am Ende | Batch-Vorschlag + Chat-URL in Prompt einbetten |
| Alle Unteraufgaben done | Auswahlfrage: Parent auch schliessen? |
| "notiz" / "nicht vergessen" | Auswahlfrage: Task, Notiz, Beides |
...
### code-erstellen
Nach Commit: Auswahlfrage: "Passt zu <PRÄFIX>-NNN. `tracker done`?" → Ja / Nein / Anderer Task
...
### doc-pflege
Nach Doc-Update: Auswahlfrage: "Doc-Task <PRÄFIX>-NNN erledigt. `tracker done`?" → Ja / Nein
...
- Titel ohne `<PRÄFIX>-NNN` oder Kürzel (Hauptaufgaben; Präfix aus der Projekt-Config)
...
- **Prosa-Fragen bei festen Entscheidungsoptionen** — IMMER Auswahlfrage verwenden
...
- **Stale-Check-Ergebnis automatisch aus Memory entfernen** ohne Auswahlfrage Bestätigung
...
- **Nach `tracker done` passiv auf User-Input warten** ohne Auswahlfrage mit Folgeoptionen
- **Commit-Hash vom User anfordern** wenn eine Shell (DC oder Bash) verfügbar ist — Hash selbst via `git log -1 --format='%h %s'` holen
- **Zwischenstand-Tabelle nach Task-Abschluss weglassen** — Übersicht über erledigte Items und Push-Pending ist Pflicht
- **Folgeoptionen als Prosa-Frage statt Auswahlfrage** — "Was willst du als nächstes?" im Fließtext ist VERBOTEN, nur Button-Optionen
